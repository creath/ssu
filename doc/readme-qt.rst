SSUnitedCoin-qt: Qt4 GUI for SSUnitedCoin
===============================
...
Alternatively, install Qt Creator and open the `SSUnitedCoin-qt.pro` file.

An executable named `SSUnitedCoin-qt` will be built.
...
A warning for people using the *static binary* version of SSUnitedCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of SSUnitedCoin is linked against libdb5.3.
...
installed causes SSUnitedCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
