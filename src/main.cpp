...
unsigned int nStakeTargetSpacing = 2 * 60; // 120-minute stakes spacing
...
const string strMessageMagic = "SSUnitedCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(unsigned int nBits, int64_t nFees, int nHeight)
{
    ...
    int halvings = nHeight / 3571429;
    if (halvings >= 64)
        return nFees;
		
	if (nHeight == 1)
		return 22450000000 * COIN;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	int64_t nTargetTimespan = 2 * 15 * 60;
	
	// Fix
	if (pindexLast->nHeight < 32) {
		nTargetSpacing = 120 * 60;
		nTargetTimespan = 120 * 15 * 60; }
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("SSUnitedCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in SSUnitedCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // SSUnitedCoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SSUnitedCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "ssuc genesis";
        CTransaction txNew;
        txNew.nTime = 1477058888;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1477058889;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1634998; // !fTestNet ? 1575379 : 46534;
		
  
        block.print();

        //// debug print
        assert(block.hashMerkleRoot == uint256("0x5189b9b153c234bf6bc2572c18947509f31c14453e9af6bce090f064ecd05e42"));
