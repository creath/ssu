...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("SSUnitedCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SSUnitedCoinTEST2\r");
            Send(hSocket, "WHO #SSUnitedCoinTEST2\r");
        } else {
            // randomly join #SSUnitedCoin00-#SSUnitedCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #SSUnitedCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SSUnitedCoin%02d\r", channel_number).c_str());
        }
